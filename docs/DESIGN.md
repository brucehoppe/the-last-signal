...
- **Guards think.** `Awareness` (Idle, Alert, Searching), last-seen pursuit with breadth-first pathing, posts to return to, a sentinel leash, hunter tracking, and alerts that spread within `SIGNAL_RANGE`. On top of that, rule-based tactics (no model): a second alert guard **flanks** by working round you and the lead unit rather than queuing behind it, and keeps going while out of sight (`flank_route`, `keep_flanking`); sentinels **hold corridors** on your way to their post (`choke_goal`); hunters **sweep** nearby rooms while searching (`sweep_waypoint`); a badly hurt sentinel **falls back** to an ally; searching guards check the corners where you **ambushed** a unit (`ambush_spots`); the Overseer **calls reinforcements** to the relay (`overseer_calls`). Each guard also has seeded habits (`Game::roll`): its breaking point, whether it holds or flanks this time, whether it recalls a corner, and whether it misses you at the edge of its sight all vary, so the rules cannot simply be memorised. The rolls hash the seed, floor, turn and guard number, so a run still replays exactly and a daily seed is the same for everyone. `Game::predictable` switches them off for tests.
